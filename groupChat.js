...
let members = document.querySelector('.dropdown-list .members');
...
let groupId = document.querySelector('.details .groupId');
let outgoing_id = document.querySelector('.typing-area .outgoing_id');
...
function getGroupMemberList(){
    ...
    xhr.open('POST', 'php/getGroupMembers.php', true);

    xhr.onload = () => {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            if (xhr.status == 200) {
                modalUserList.innerHTML = xhr.response;
            }
        }
    }
    xhr.send(formData);
...
function deleteGroupMember(memberId){
    ...
    xhr.open('POST', 'php/deleteGroupMember.php', true);
    xhr.onload = () => {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            if (xhr.status == 200) {
                getGroupMemberList();
...
function sendMessage(formData) {
    ...
    xhr.open('POST', 'php/groupChat.php', true);
...
let mssg = setInterval(() => {
    ...
    xhr.open('POST', 'php/getGroupMessage.php', true);
